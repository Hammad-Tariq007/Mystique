  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-offwhite via-white to-peach-50 dark:from-neutral-900 dark:via-black dark:to-neutral-950 relative">
      ...
      <div className="w-full max-w-md mx-auto rounded-2xl shadow-2xl bg-white/95 dark:bg-neutral-900/95 px-8 py-12 flex flex-col gap-8 items-center animate-fade-in-scale">
        <h1 className="text-4xl font-serif font-bold tracking-tight text-black dark:text-white mb-2 animate-fade-up">Welcome Back</h1>
        <p className="text-base text-gray-500 dark:text-gray-400 mb-6 animate-fade-up delay-100">Sign in to continue your Mystique journey.</p>
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="w-full flex flex-col gap-6">
          ...
          {loginError && <p className="text-red-500 text-sm w-full text-center animate-fade-up delay-400">{loginError}</p>}
          {/* Forgot Password */}
          <button
            type="button"
            className={`text-gold/80 hover:text-gold underline underline-offset-4 transition-all duration-200 w-fit mx-auto text-xs font-medium animate-fade-up delay-400 ${highlightForgot ? 'animate-shake' : ''}`}
          ...
          <div className="w-full text-center mt-2 animate-fade-up delay-600">
            ...
            <Link to='/signup' className="text-gold font-semibold hover:underline underline-offset-4 transition-all">Create one here</Link>
          </div>
        </form>
      </div>
